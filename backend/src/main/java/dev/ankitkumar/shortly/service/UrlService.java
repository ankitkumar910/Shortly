...
public class UrlService {
    ...
    public UrlService(UrlRepository repository, UrlShortener urlShortener) {
    ...
    public String shorten(   String url,@Pattern(regexp = "^[0-9a-zA-Z]+",message = "Invalid short code provided.")  String customShortCode) {


        if(url.isBlank() || !(url.startsWith("https://") || url.startsWith("http://"))) throw  new IllegalArgumentException("Long url is not valid.");
        ...
        if(customShortCode != null && !customShortCode.isBlank()){
            if(repository.existsByShortUrl(customShortCode)) throw  new DuplicateEntryException("Provided custom short code is not available.");
    ...
    public String longUrl(@NotNull(message = "Short code is required.") @Pattern(regexp = "^[0-9a-zA-Z]+",message = "Invalid short code provided.")  String shortCode) {
